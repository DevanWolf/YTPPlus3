...
if ($args.Length -lt 13) {
    ...
    Write-Host "Usage: spartaremix.ps1 <video> <width> <height> <temp> <ffmpeg> <ffprobe> <magick> <resources> <sounds> <sources> <music> <library> <options>"
    exit 1
}
...
.\ffmpeg.exe -i "$temp1" -ss 0 -t 0.100 -y "$temp2"
.\ffmpeg.exe -i "$temp1" -ss 0.140 -t 0.050 -y "$temp3"
